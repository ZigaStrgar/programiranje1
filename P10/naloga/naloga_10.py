rodbina = {"Kajn": ["Adam"], "Adam": ["Matjaž", "Cilka", "Daniel", "Erik"], "Aleksander": [], "Alenka": [],
           "Barbara": [], "Cilka": [], "Daniel": ["Elizabeta", "Hans"], "Erik": [],
           "Elizabeta": ["Ludvik", "Jurij", "Barbara", "Herman"], "Franc": [], "Herman": ["Margareta"], "Hans": [],
           "Jožef": ["Alenka", "Aleksander", "Petra"], "Jurij": ["Franc", "Jožef"], "Ludvik": [], "Margareta": [],
           "Matjaž": ["Viljem"], "Mihael": [], "Petra": [], "Tadeja": [], "Viljem": ["Tadeja"],}
...
starost = {"Kajn": 140, "Adam": 111, "Matjaž": 90, "Cilka": 88, "Daniel": 85, "Erik": 83, "Viljem": 58, "Tadeja": 20,
           "Elizabeta": 67, "Hans": 64, "Ludvik": 50, "Jurij": 49, "Barbara": 45, "Herman": 39, "Mihael": 32,
           "Franc": 30, "Jožef": 29, "Margareta": 3, "Alenka": 9, "Aleksander": 5, "Petra": 7}


def starost_starsa(oseba):
    if otroci(oseba):
        return starost[oseba] - starost[otroci(oseba)[0]]
    else:
        return 999
...
def najmlajsi_stars(oseba):
    ...
    razlika = starost_starsa(oseba)
    for otrok in moji_otroci:
        razlika2 = najmlajsi_stars(otrok)
        if razlika2 < razlika:
            razlika = razlika2
    return razlika
...
def starsi_pred(oseba, n):
    ...
    moji_otroci = otroci(oseba)
    for otrok in moji_otroci:
        imena = imena | starsi_pred(otrok, n)
    return imena
...
class TestOgrevanje(unittest.TestCase):
    ...
    def test_najmlajsi_stars(self):
        ...
        self.assertEqual(len([x for x in najmlajsi_stars.call_args_list if x[0][0] == "Elizabeta"]), 1,
                         "Klic najmlajsi_stars('Elizabeta') se mora zgoditi natančno enkrat")


class TestObvezna(unittest.TestCase):
    def test_starsi_pred(self):
        self.assertEqual(starsi_pred("Adam", 20), {"Daniel", "Elizabeta", "Jurij"})
        self.assertEqual(starsi_pred("Daniel", 20), {"Daniel", "Elizabeta", "Jurij"})
        ...
        self.assertEqual(starsi_pred("Adam", 50),
                         {"Adam", "Matjaž", "Viljem", "Daniel", "Elizabeta", "Jurij", "Herman", "Jožef"})
