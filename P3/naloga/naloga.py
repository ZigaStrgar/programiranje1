...
x = float(input("Vnesi koordinato x: "))
y = float(input("Vnesi koordinato y: "))

max_distance = 0
max_distance_name = ""
for name, lat, lng in kraji:
    distance = abs(sqrt((x - lat) ** 2 + (y - lng) ** 2))
    if max_distance < distance:
        max_distance = distance
        max_distance_name = name
print(max_distance_name)
# Konec obvezne naloge
...
last_lng = -1000
"""
last_lng = last_lat = None
for name, lat, lng in kraji:
    """
    if last_lat == -1000 and last_lng == -1000:
        last_lat = lat
        last_lng = lng
    distance_traveled += abs(sqrt((last_lat - lat) ** 2 + (last_lng - lng) ** 2))
    last_lng = lng
    last_lat = lat
    """
    if last_lat != None:
...
print("Prepotoval bi:", distance_traveled, "enot")
# Konec dodatne naloge

print("\n------------------\n")

# Zelo dodatna naloga
# Problem potujočega potnika, žal trenutno presega moje sposobnosti v Pythonu in tudi računalnik bi veliko časa računal vse možnosti :)
# https://github.com/dmishin/tsp-solver
# https://github.com/google/or-tools/blob/master/examples/python/tsp.py
# Konec zelo dodatne naloge

# Rešitev zelo dodatne naloge
