...
def najblizji(kraj, kraji):
    ...
    for ime, x2, y2 in kraji:
        razdalja_st = razdalja(x, y, x2, y2)
        if razdalja_st != 0:
            if najblizje is None:
                najblizje = razdalja_st
                ime_kraja = ime
            if najblizje > razdalja_st:
                najblizje = razdalja_st
                ime_kraja = ime
    return ime_kraja
...
def vojska(kraji):
    pot = []
    shortest_path = veriga("Brežice", kraji)

    for i in range(len(shortest_path) - 1):
        pot.append((shortest_path[i], shortest_path[i + 1]))
    return pot
