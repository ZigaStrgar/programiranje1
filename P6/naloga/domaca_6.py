...
def barvanje(barve):
    sahovnica = collections.defaultdict(list)
...
def prestej_barve(sahovnica):
    sez = collections.defaultdict(int)
    for polje, barva_polja in sahovnica.items():
        sez[barva_polja] += 1
    return sez
...
def polja_po_barvah(sahovnica):
    sez = collections.defaultdict(dict)
    for polje, barva_polja in sahovnica.items():
        sez[barva_polja] = polja_po_barvi(sahovnica, barva_polja)
    return sez
